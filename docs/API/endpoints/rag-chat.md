...
When the configured LLM is unreachable (e.g. Ollama Cloud/API down, local Ollama not running, OpenAI API key missing/invalid, or network error), the API returns **503** with a short, actionable message instead of a generic 500. The message depends on the configured provider (default is Ollama Cloud).
